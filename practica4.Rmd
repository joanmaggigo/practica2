...
recomanacio_hotels<-function(entrada, list.model.recomanacio,result_roc)
{
  ...
  for (i in 1:NROW(list.model.recomanacio))
  {
    ...
    if (predict.hotel>=(roc_actual$Optim_point)) 
      {
        result_actual<-cbind(as.character(roc_actual$NomHotel),roc_actual$nota,predict.hotel,roc_actual$Optim_point)
        colnames(result_actual)<-c("Nom Hotel","Nota","Valor Prediccio","Valor Optim")
        recomacio<-rbind(recomanacio,result_actual) 
      }
    else
    {
         result_actual<-cbind(as.character(roc_actual$NomHotel),roc_actual$nota,predict.hotel,roc_actual$Optim_point)
  ...
  return(list("recomenats"=recomacio, "norecomenats"=norecomenats))
}

#entrada<-data.frame("Habitacio Doble",3,"Viajedeocio","Pareja","PRIMER TRIMESTRE")
entrada<-data.frame("Habitacio Doble",4,"Viajedeocio","Grupo","TERCER TRIMESTRE")
